  ...
  */
    ...
    */
    ...
    var getWords = function () {
      return $('input.word1').toArray().map(wordEl => wordEl.value.toLowerCase()).filter(word => word);
    };
  ...
  */
  var WordFindGame = function (puzzleEl, options) {
    ...
    */
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          wordList.splice(i,1);
          $('input.word1[value="' + curWord + '"]').addClass('wordFound').addClass('wordFound1');
        }
    ...
    /* Constructor START */
    $('input.word1').removeClass('wordFound').removeClass('wordFound1');
    ...
    */
    this.solve = function() {
      ...
      for( var i = 0, len = solution.length; i < len; i++) {
        ...
        var wordEl = $('input.word1[value="' + word + '"]');
  ...
  WordFindGame.insertWordBefore = function (el, word) {
    $('<li><input class="word word1" value="' + (word || '') + '"></li>').insertBefore(el);
  };
...
(function (document, $, wordfind2) {
  ...
  /**
  * An example game using the puzzles created from wordfind2.js. Click and drag
  * to highlight words.
  *
  * wordfind2Game2 requires wordfind2.js and jQuery.
  */
    ...
    */
    ...
    var getWords = function () {
      return $('input.word2').toArray().map(wordEl => wordEl.value.toLowerCase()).filter(word => word);
    };
  ...
  /**
  * Initializes the wordfind2Game2 object.
  *
  * Creates a new word find game and draws the board and words.
  *
  * Returns the puzzle that was created.
  *
  * @param {String} puzzleEl: Selector to use when inserting the puzzle
  * @param {Options} options: wordfind2 options to use when creating the puzzle
  */
  var wordfind2Game2 = function (puzzleEl, options) {
    ...
    */
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          $('.selected').addClass('found');
          wordList.splice(i,1);
          $('input.word2[value="' + curWord + '"]').addClass('wordFound').addClass('wordFound2');
        }
    ...
    /* Constructor START */
    $('input.word2').removeClass('wordFound').removeClass('wordFound2');

    // Class properties, game initial config:
    wordList = getWords().sort();
    puzzle = wordfind2.newPuzzleLax(wordList, options);
    ...
    */
    this.solve = function() {
      var solution = wordfind2.solve(puzzle, wordList).found;
      ...
      for( var i = 0, len = solution.length; i < len; i++) {
        ...
        var wordEl = $('input.word2[value="' + word + '"]');
        if (!wordEl.hasClass('wordFound')) {
          for (var j = 0, size = word.length; j < size; j++) {
            var nextPos = next(x, y, j);
            $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').addClass('solved');
  ...
  wordfind2Game2.emptySquaresCount = function () {
    var allSquares = $('.puzzleSquare').toArray();
    return allSquares.length - allSquares.filter(b => b.textContent.trim()).length;
  };

  // Static method
  wordfind2Game2.insertWordBefore = function (el, word) {
    //$('<div><input class="word" value="' + (word || '') + '"></div>').insertBefore(el);
    $('<li><input class="word word2" value="' + (word || '') + '"></li>').insertBefore(el);
    //$('<li><input class="word" value="' + (word || '') + '"></li>').insertAfter(el);
    //$('<li><input class="word" value="' + (word || '') + '"></li>').insert(el);
  };
  ...
  * Allow game to be used within the browser
  */
  window.wordfind2Game2 = wordfind2Game2;
