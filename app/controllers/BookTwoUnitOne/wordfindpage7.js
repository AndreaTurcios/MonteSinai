var A1,J1

//palabra neck color rojo
function comprobar1(){
	a1 = document.getElementById('celda1').style.color="red";
}
function comprobarB1(){
	a1 = document.getElementById('celdaB1').style.color="red";
}
function comprobarC1(){
	a1 = document.getElementById('celdaC1').style.color="red";
}
function comprobarD1(){
	a1 = document.getElementById('celdaD1').style.color="red";
}

//palabra hand color azul
function comprobar10(){
	b2 = document.getElementById('celda10').style.color="blue";
}
function comprobarB10(){
	b2 = document.getElementById('celdaB10').style.color="blue";
}
function comprobarC10(){
	b2 = document.getElementById('celdaC10').style.color="blue";
}
function comprobarD10(){
	b2 = document.getElementById('celdaD10').style.color="blue";
}

//palabra head color purple 
function comprobarB4(){
	c3 = document.getElementById('celdaB4').style.color="purple";
}
function comprobarC4(){
	c3 = document.getElementById('celdaC4').style.color="purple";
}
function comprobarD4(){
	c3 = document.getElementById('celdaD4').style.color="purple";
}
function comprobarE4(){
	c3 = document.getElementById('celdaE4').style.color="purple";
}

//palabra foot color green 
function comprobarC2(){
	d4 = document.getElementById('celdaC2').style.color="green";
}
function comprobarD2(){
	d4 = document.getElementById('celdaD2').style.color="green";
}
function comprobarE2(){
	d4 = document.getElementById('celdaE2').style.color="green";
}
function comprobarF2(){
	d4 = document.getElementById('celdaF2').style.color="green";
}

//palabra ear color darkblue 
function comprobarC5(){
	e5 = document.getElementById('celdaC5').style.color="darkblue";
}
function comprobarD5(){
	e5 = document.getElementById('celdaD5').style.color="darkblue";
}
function comprobarE5(){
	e5 = document.getElementById('celdaE5').style.color="darkblue";
}

//palabra eye color fuchsia  
function comprobarC7(){
	f6 = document.getElementById('celdaC7').style.color="fuchsia";
}
function comprobarD7(){
	f6 = document.getElementById('celdaD7').style.color="fuchsia";
}
function comprobarE7(){
	f6 = document.getElementById('celdaE7').style.color="fuchsia";
}

//palabra arm color silver
function comprobarE6(){
	g7 = document.getElementById('celdaE6').style.color="silver";
}
function comprobarF6(){
	g7 = document.getElementById('celdaF6').style.color="silver";
}
function comprobarG6(){
	g7 = document.getElementById('celdaG6').style.color="silver";
}

//palabra leg color olive
function comprobarE9(){
	h8 = document.getElementById('celdaE9').style.color="olive";
}
function comprobarF9(){
	h8 = document.getElementById('celdaF9').style.color="olive";
}
function comprobarG9(){
	h8 = document.getElementById('celdaG9').style.color="olive";
}

//palabra mouth color #5D6D7E
function comprobarF3(){
	h8 = document.getElementById('celdaF3').style.color="#5D6D7E";
}
function comprobarG3(){
	h8 = document.getElementById('celdaG3').style.color="#5D6D7E";
}
function comprobarH3(){
	h8 = document.getElementById('celdaH3').style.color="#5D6D7E";
}
function comprobarI3(){
	h8 = document.getElementById('celdaI3').style.color="#5D6D7E";
}
function comprobarJ3(){
	h8 = document.getElementById('celdaJ3').style.color="#5D6D7E";
}

//palabra nose color #B03A2E
function comprobarF8(){
	h8 = document.getElementById('celdaF8').style.color="#B03A2E";
}
function comprobarG8(){
	h8 = document.getElementById('celdaG8').style.color="#B03A2E";
}
function comprobarH8(){
	h8 = document.getElementById('celdaH8').style.color="#B03A2E";
}
function comprobarI8(){
	h8 = document.getElementById('celdaI8').style.color="#B03A2E";
}
